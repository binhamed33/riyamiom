<?php

return [
    'case_accounting' => 'Accounting',
    'total_fees' => 'Total fees',
    'total_invoices' => 'Total invoices',
    'collected' => 'Collected',
    'outstanding' => 'Outstanding',
    'add' => 'Add',
    'add_case_fee' => 'Add a case fee',
    'fee_type' => 'Fee type',
    'amount' => 'Amount',
    'paid' => 'Paid',
    'unpaid' => 'Unpaid',
    'show_to_client' => 'Client can see',
    'visible_to_client' => 'Visible to client',
    'case_fees' => 'Fees',
    'case_invoices' => 'Invoices',
    'no_fees_yet' => 'No fees recorded on this case yet.',
    'currency_omr' => 'OMR',
    'accounting_note' => 'These items belong to the office finance section and appear in its reports.',
    'open_finance' => 'Open finance',
    'efficiency_comparison_help' => 'Three measures per member side by side: overall efficiency, share of decided cases won, and share of tasks completed. Taller is better.',
    'monthly_case_trends_help' => 'Office movement across recent months: cases opened, won and lost — read the trend, not a single figure.',
    'cases_by_type_help' => 'Where the work actually concentrates: the top three case types, the rest grouped as Other.',
    'team_comparison_help' => 'One group of bars per measure, bar length is the percentage — read across a row for who leads and by how much; a short row is where the team needs support.',
    'show_numbers' => 'Show the numbers',
    'no_data_yet' => 'Not enough data yet — this panel fills in as cases progress.',
    'details' => 'Details',
    'thumbnails' => 'Thumbnails',
    'folders' => 'Folders',
    'no_folders_yet' => 'No folders yet',
    'manage_folders' => 'Manage folders',
    'done_managing' => 'Done',
    'new_folder' => 'New folder',
    'new_folder_name' => 'New folder name',
    'rename' => 'Rename',
    'delete_folder_confirm' => 'Delete this folder? Its documents move to General; nothing is deleted.',
    'move_to_folder' => 'Move to folder',
    'folder_general' => 'General (no folder)',
    'view_details' => 'Details view — table with name, date, type and size',
    'view_thumbnails' => 'Thumbnails view — large tiles with name and date',
    'sort_newest' => 'Newest',
    'file_type' => 'Type',
    'file_size' => 'Size',
    'action' => 'Action',
    'user' => 'User',
    'sort_by' => 'Sort by',
    'show_done' => 'Completed',
    'show_active' => 'Show active',
    'done_cases_btn' => 'Completed cases',
    'of_total' => 'of total',
    'won_cases' => 'won',
    'lost_cases' => 'lost',

    // مفاتيح كانت تظهر خاماً في الواجهة (app.total وأخواتها)
    'total' => 'Total',
    'open' => 'Open',
    'calendar' => 'Calendar',
    'session' => 'Session',


    // General
    'dashboard' => 'Dashboard',
    'cases' => 'Cases',
    'sessions' => 'Sessions',
    'tasks' => 'Tasks',
    'documents' => 'Documents',
    'clients' => 'Clients',
    'chat' => 'Chat',
    'chat_users' => 'Employees',
    'users' => 'Users',
    'settings' => 'Settings',
    'audit_log' => 'Audit Log',
    'notifications' => 'Notifications',
    'feasibility_study' => 'Feasibility Study',
    'logout' => 'Logout',
    'login' => 'Login',
    'save' => 'Save',
    'cancel' => 'Cancel',
    'delete' => 'Delete',
    'edit' => 'Edit',
    'create' => 'Create',
    'view' => 'View',
    'search' => 'Search',
    'filter' => 'Filter',
    'actions' => 'Actions',
    'confirm_delete' => 'Are you sure you want to delete?',
    'success' => 'Operation completed successfully',
    'error' => 'An error occurred',
    'warning' => 'Warning',
    'no_data' => 'No data available',
    'loading' => 'Loading...',
    'back' => 'Back',
    'next' => 'Next',
    'previous' => 'Previous',
    'home' => 'Home',
    'reset' => 'Reset',
    'apply' => 'Apply',
    'all' => 'All',
    'name' => 'Name',
    'type' => 'Type',
    'status' => 'Status',
    'priority' => 'Priority',
    'description' => 'Description',
    'title' => 'Title',
    'date' => 'Date',
    'phone' => 'Phone',
    'phone_country' => 'Country',
    'phone_country_search' => 'Search country or code…',
    'phone_country_none' => 'No country by that name',

    'email' => 'Email',
    'address' => 'Address',
    'notes' => 'Notes',
    'location' => 'Location',
    'court' => 'Court',
    'confirm_restore' => 'Are you sure you want to restore this?',
    'no_results' => 'No results found',
    'add_new' => 'Add New',
    'back_to_list' => 'Back to List',
    'manual_entry' => 'Manual Entry',
    'from_date' => 'From Date',
    'to_date' => 'To Date',
    'select_date' => 'Select Date',

    // Navigation
    'main_section' => 'Main',
    'admin_section' => 'Administration',

    // Status labels
    'status_active' => 'Active',
    'status_pending' => 'Pending',
    'status_overdue' => 'Overdue',
    'status_closed' => 'Closed',
    'status_won' => 'Won',
    'status_lost' => 'Lost',
    'status_adjudicated' => 'Adjudicated',
    'status_fees_pending' => 'Fees Pending',

    // Priority labels
    'priority_low' => 'Low',
    'priority_medium' => 'Medium',
    'priority_high' => 'High',
    'priority_urgent' => 'Urgent',

    // Session status
    'status_upcoming' => 'Upcoming',
    'status_completed' => 'Completed',
    'status_postponed' => 'Postponed',
    'status_cancelled' => 'Cancelled',
    'status_held' => 'Held',
    'status_scheduled' => 'Scheduled',

    // Task status
    'status_in_progress' => 'In Progress',

    // Cases
    'case_number' => 'Case Number',
    'case_title' => 'Case Title',
    'case_description' => 'Case Description',
    'case_type' => 'Case Type',
    'case_court' => 'Court',
    'case_court_with_number' => 'Court & Case No.',
    'case_opponent' => 'Opponent',
    'client' => 'Client',
    'case_status' => 'Case Status',
    'case_priority' => 'Priority',
    'case_opened_at' => 'Date Opened',
    'case_next_date' => 'Next Date',
    'case_client' => 'Client',
    'case_lawyer' => 'Lawyer',
    'add_case' => 'Add Case',
    'edit_case' => 'Edit Case',
    'delete_case' => 'Delete Case',
    'case_summary' => 'Case Summary',
    'print' => 'Print',
    'manage_cases' => 'Manage Cases',
    'overdue_report' => 'Overdue Report',
    'add_new_case' => 'Add New Case',
    'basic_info' => 'Basic Information',
    'case_number_placeholder' => 'e.g. 2026/1234',
    'case_number_auto' => 'A random number will be generated if left empty',
    'case_title_placeholder' => 'Case title',
    'case_type_placeholder' => 'e.g. Civil, Criminal, Commercial',
    'opponent_name_placeholder' => 'Opponent name',
    'case_description_placeholder' => 'Detailed description of the case...',
    'choose_court' => 'Choose Court',
    'court_manual_placeholder' => 'Type court name manually',
    'case_status_priority' => 'Status & Priority',
    'case_dates' => 'Dates',
    'opened_date' => 'Opened Date',
    'next_date' => 'Next Date',
    'next_session_date' => 'Next Session Date',
    'related_people' => 'Related People',
    'court_case_number' => 'Case Number in Court',
    'office_case_number' => 'Office Case Number',
    'office_case_number_auto' => 'Auto-generated and cannot be edited',
    'office_case_number_placeholder' => 'Case number in office',
    'case_title_number' => 'Case Title/Number',
    'case_title_number_placeholder' => 'Case title or short identifier',
    'court_with_case_number' => 'Court & Case Number',
    'case_principal' => 'Principal',
    'choose_case_type' => 'Choose Case Type',
    'opponent_data' => 'Opponent Data',
    'opponent_name' => 'Opponent Name',
    'opponent_phone' => 'Phone Number',
    'opponent_phone_placeholder' => 'Opponent phone',
    'opponent_address' => 'Address',
    'opponent_address_placeholder' => 'Opponent address',
    'opponent_lawyer' => 'Opponent Lawyer',
    'opponent_lawyer_placeholder' => 'Opponent lawyer name',
    'opponent_civil_number' => 'Civil Number',
    'opponent_email' => 'Opponent email',
    'opponent_role' => 'Opponent capacity',
    'opponent_type' => 'Opponent type',
    'opponent_notes' => 'Opponent notes',
    'opponent_civil_number_placeholder' => 'Opponent civil number',
    'session_decision' => 'Session Decision',
    'session_decision_placeholder' => 'Write the session decision...',
    'view_decision' => 'View Decision',
    'add_decision' => 'Add Decision',
    'employee' => 'Employee',
    'add_session' => 'Add Session',
    'choose_client' => 'Choose Client',
    'choose_lawyer' => 'Choose Lawyer',
    'add_case_button' => 'Add Case',
    'save_changes' => 'Save Changes',
    'confirm_delete_case' => 'Are you sure you want to delete this case?',
    'confirm_delete_case_full' => 'Are you sure you want to delete this case? All related sessions, tasks and documents will be deleted.',
    'case_details' => 'Case Details',
    'download_case_pdf' => 'Download Case File PDF',
    'sessions_tab' => 'Sessions',
    'tasks_tab' => 'Tasks',
    'documents_tab' => 'Documents',
    'table_date' => 'Date',
    'table_type' => 'Type',
    'table_hall' => 'Hall',
    'table_judge' => 'Judge',
    'table_notes' => 'Notes',
    'no_sessions_recorded' => 'No sessions recorded',
    'deadline' => 'Deadline:',
    'no_tasks_recorded' => 'No tasks recorded',
    'documents_count' => 'document|documents',
    'no_documents_attached' => 'No documents attached',
    'print_summary' => 'Print',
    'copy_summary' => 'Copy Summary',
    'summary_copied' => 'Summary copied!',
    'no_cases' => 'No cases',
    'add_case_prompt' => 'Add a new case',
    'no_trashed_cases' => 'No trashed cases',
    'restore' => 'Restore',
    'restore_case_confirm' => 'Are you sure you want to restore this case?',
    'back_to_cases' => 'Back to Cases',

    // Sessions
    'session_date' => 'Session Date',
    'session_location' => 'Session Location',
    'session_status' => 'Session Status',
    'session_notes' => 'Session Notes',
    'add_session' => 'Add Session',
    'new_session' => 'New Session',
    'choose_case' => 'Choose Case',
    'session_datetime' => 'Date & Time',
    'session_location_placeholder' => 'e.g. Primary Court',
    'session_notes_placeholder' => 'Add session notes...',
    'session_details' => 'Session Details',
    'session_case_info' => 'Related Case Info',
    'view_case' => 'View Case',
    'no_linked_case' => 'No linked case.',
    'confirm_delete_session' => 'Are you sure you want to delete this session?',
    'delete_session' => 'Delete Session',
    'no_sessions' => 'No sessions.',

    // Tasks
    'task_title' => 'Task Title',
    'task_description' => 'Task Description',
    'task_assigned_to' => 'Assigned To',
    'task_status' => 'Task Status',
    'task_priority' => 'Task Priority',
    'task_due_date' => 'Due Date',
    'add_task' => 'Add Task',
    'new_task' => 'New Task',
    'task_title_placeholder' => 'Task title',
    'task_description_placeholder' => 'Detailed description...',
    'no_case' => 'No Case',
    'choose_assignee' => 'Choose Assignee',
    'due_date' => 'Due Date',
    'task_details' => 'Task Details',
    'created_by' => 'Created By',
    'creation_date' => 'Creation Date',
    'completion_date' => 'Completion Date',
    'overdue' => 'Overdue',
    'other' => 'Other',
    'linked_case' => 'Linked Case',
    'change_status' => 'Change Status',
    'start_progress' => 'Start Progress',
    'complete_task' => 'Complete Task',
    'confirm_delete_task' => 'Are you sure you want to delete this task?',
    'delete_task' => 'Delete Task',
    'no_tasks' => 'No tasks.',

    // Documents
    'document_title' => 'Document Title',
    'document_file' => 'Document File',
    'document_access' => 'Access Level',
    'upload_document' => 'Upload Document',
    'new_document' => 'Upload New Document',
    'document_title_placeholder' => 'Document title',
    'allowed_formats' => 'Allowed formats: PDF, DOC, DOCX, XLS, XLSX, JPG, PNG - Max size: 20MB',
    'no_case' => 'No Case',
    'access_public' => 'Public',
    'access_team' => 'Team Only',
    'access_private' => 'Private',
    'upload' => 'Upload',
    'uploaded_by' => 'Uploaded By',
    'table_size' => 'Size',
    'table_access' => 'Access',
    'table_uploaded' => 'Uploaded',
    'download' => 'Download',
    'preview' => 'Preview',
    'no_documents' => 'No documents.',
    'file_not_found' => 'File not found on server.',

    // Clients
    'client_name' => 'Client Name',
    'client_type' => 'Client Type',
    'client_phone' => 'Client Phone',
    'client_email' => 'Client Email',
    'client_address' => 'Client Address',
    'individual' => 'Individual',
    'company' => 'Company',
    'add_client' => 'Add Client',
    'new_client' => 'New Client',
    'documents_new' => 'new documents',
    'client_full_name' => 'Full Name',
    'client_name_placeholder' => 'Client name',
    'client_phone_placeholder' => 'Phone number',
    'client_email_placeholder' => 'Email address',
    'client_national_id' => 'National ID',
    'national_id_placeholder' => 'National ID number',
    'client_address_placeholder' => 'Client address',
    'company_name' => 'Company Name',
    'client_details' => 'Client Info',
    'registered_cases' => 'Registered Cases',
    'client_cases' => 'Client Cases',
    'created_at' => 'Created At',
    'no_client_cases' => 'No registered cases',
    'no_clients' => 'No clients',
    'no_trashed_clients' => 'No trashed clients',
    'back_to_clients' => 'Back to Clients',
    'restore_client_confirm' => 'Are you sure you want to restore this client?',
    'new_client_button' => '+ New Client',
    'clear_filters' => 'Clear filters',
    'quick_range' => 'Period',
    'range_today' => 'Today',
    'range_week' => 'This week',
    'range_month' => 'This month',
    'all_courts' => 'All courts',
    'all_cases' => 'All cases',
    'all_lawyers' => 'All lawyers',
    'all_types' => 'All types',
    'due_filter' => 'Due',
    'due_overdue' => 'Overdue',
    'due_today' => 'Due today',
    'due_week' => 'Within a week',
    'due_upcoming' => 'Upcoming',
    'tab_all' => 'All',
    'tab_today' => 'Today',
    'tab_upcoming' => 'Upcoming',
    'tab_overdue' => 'Overdue',
    'tab_completed' => 'Completed',
    'registered_from' => 'Registered from',
    'registered_to' => 'Registered to',
    'clear' => 'Clear',
    'search_placeholder' => 'Search by name, phone, email...',

    // Users
    'user_name' => 'User Name',
    'user_email' => 'User Email',
    'user_role' => 'User Role',
    'user_phone' => 'User Phone',
    'user_status' => 'User Status',
    'developer' => 'Developer',
    'admin' => 'Admin',
    'lawyer' => 'Lawyer',
    'staff' => 'Staff',
    'client_role' => 'Client',
    'active' => 'Active',
    'inactive' => 'Inactive',
    'add_user' => 'Add User',
    'new_user' => 'New User',
    'user_full_name' => 'Full Name',
    'password' => 'Password',
    'confirm_password' => 'Confirm Password',
    'choose_role' => 'Choose Role',
    'active_account' => 'Active Account',
    'no_users' => 'No users',
    'search_users_placeholder' => 'Search by name, email...',
    'all_roles' => 'All Roles',
    'back_to_users' => 'Back to Users',
    'password_leave_empty' => 'Leave empty if you don\'t want to change',
    'new_password' => 'New Password',

    // User Stats
    'lawyer_stats' => 'Lawyer Statistics',
    'active_cases_count' => 'Active Cases',
    'assigned_tasks' => 'Assigned Tasks',
    'efficiency' => 'Efficiency',

    // Settings
    'office_name' => 'Office Name',
    'office_phone' => 'Office Phone',
    'office_email' => 'Office Email',
    'office_address' => 'Office Address',
    'office_website' => 'Office Website',
    'office_info' => 'Office Information',
    'notification_settings' => 'Notification Settings',
    'email_notifications' => 'Email Notifications',
    'email_notifications_desc' => 'Enable email notifications',
    'task_reminders' => 'Task Reminders',
    'task_reminders_desc' => 'Remind about tasks before deadline',
    'deadline_alerts' => 'Deadline Alerts',
    'deadline_alerts_desc' => 'Deadline approaching alerts',
    'system_settings' => 'System Settings',
    'timezone' => 'Timezone',
    'oman_only' => 'System is for Sultanate of Oman only',
    'date_format' => 'Date Format',
    'items_per_page' => 'Items Per Page',
    'save_settings' => 'Save Settings',

    // Settings - Timezone
    'oman_muscat' => 'Muscat - Oman (GMT+4)',

    // Settings - Date Format
    'format_gregorian' => 'Gregorian (Y/m/d)',
    'format_hijri' => 'Hijri (Y/m/d)',

    // Audit Log
    'audit_action' => 'Action',
    'audit_user' => 'User',
    'audit_model' => 'Model',
    'audit_date' => 'Date',
    'all_users' => 'All Users',
    'all_actions' => 'All Actions',
    'model_name' => 'Model',
    'from_date_label' => 'From Date',
    'to_date_label' => 'To Date',
    'apply_filter' => 'Apply Filter',
    'reset_filter' => 'Reset Filter',
    'table_datetime' => 'Date & Time',
    'table_action' => 'Action',
    'table_details' => 'Details',
    'table_ip' => 'IP',
    'system' => 'System',
    'no_logs' => 'No logs',

    // Notifications
    'mark_all_read' => 'Mark all as read',
    'no_notifications' => 'No notifications',
    'view_all_notifications' => 'View all notifications',
    'mark_as_read' => 'Mark as read',
    'notification_default' => 'Notification',

    // Feasibility
    'top_performer' => 'Top Performer',
    'efficiency_rate' => 'Efficiency Rate',
    'leaderboard' => 'Leaderboard',
    'success_rate' => 'Success Rate',
    'task_completion' => 'Task Completion',
    'deadline_compliance' => 'Deadline Compliance',
    'productivity' => 'Productivity',
    'performance_details' => 'Performance Details',
    'no_data_available' => 'No data available',

    // Profile
    'profile' => 'Profile',
    'change_password' => 'Change Password',
    'password_leave_empty_text' => 'Leave fields empty if you don\'t want to change password',
    'current_password' => 'Current Password',
    'password_min_8' => 'Password must be at least 8 characters',
    'password_uppercase' => 'Password must contain at least one uppercase letter (A-Z)',
    'password_lowercase' => 'Password must contain at least one lowercase letter (a-z)',
    'password_digit' => 'Password must contain at least one number (0-9)',
    'password_special' => 'Password must contain at least one special character (!@#$%^&*)',
    'password_requirements' => 'Password requirements: 8 characters, uppercase, lowercase, number, special character',

    // Backup
    'backup' => 'Backup',
    'create_backup' => 'Create Backup',
    'backup_desc' => 'Download a backup of the database and files',
    'manage_backups' => 'Manage system backups',
    'no_backups' => 'No backups',
    'create_first_backup' => 'Create a backup to get started',
    'backup_file_name' => 'File Name',
    'backup_size' => 'Size',
    'backup_date' => 'Date',
    'confirm_restore_backup' => 'Are you sure you want to restore this backup? The current database will be replaced.',
    'confirm_delete_backup' => 'Are you sure you want to delete this backup?',
    'backup_important_title' => 'Important: Download a backup to your computer',
    'backup_important_text' => 'Backups on the server will not protect you if the server fails. Download the backup file and keep it on your computer or a safe location.',
    'auto_backup_enabled' => 'Automatic backup enabled',
    'auto_backup_schedule' => 'A backup is created automatically daily at 3:00 AM — Last 30 backups kept',
    'backup_count' => 'backups',
    'backup_auto_keep_30' => 'Keeps last 30 backups automatically',
    'upload_backup_file' => 'Choose a backup file from your computer',
    'restore_upload' => 'Restore from file',
    'restoring' => 'Restoring...',

    // Auth
    'login_title' => 'Login',
    'login_email' => 'Email Address',
    'login_password' => 'Password',
    'login_remember' => 'Remember Me',
    'login_button' => 'Sign In',
    'login_failed' => 'Invalid email or password',
    'account_locked' => 'Account is locked',
    'login_subtitle' => 'Enter your credentials to access the dashboard',
    'email' => 'Email',
    'remember_me' => 'Remember me',
    'forgot_password' => 'Forgot password?',
    'suggestions_box' => 'Suggestions Box',
    'daily_work_section' => 'Daily work',
    'records_section' => 'Reports & records',
    'developer_section' => 'Developer tools',
    'help_section' => 'Help & contact',
    'maintenance_title' => 'We are improving Mudawala',
    'maintenance_body' => 'The system is under maintenance for a short while. We will be back as soon as it is done.',
    'maintenance_data_safe' => 'Your office data is untouched and safe.',
    'maintenance_staff_access' => 'Open the admin panel',
    'appearance' => 'Appearance',
    'empty_filtered_title' => 'No matching results',
    'empty_filtered_hint' => 'No record matched the filters applied. Widen them, or clear them to see the full list.',
    'no_sessions_hint' => 'Upcoming sessions appear here as soon as you add them to a case.',
    'no_tasks_hint' => 'Tasks assigned to you and to the office team appear here.',
    'no_clients_hint' => 'Add your first client so cases can be linked to them.',
    'no_documents_hint' => 'Documents uploaded to cases and clients appear here.',
    'active_filters' => 'Active filters',
    'remove_filter' => 'Remove filter',
    'close' => 'Close',
    'client_activity' => 'Status (by cases)',
    'client_has_active_cases' => 'Has active cases',
    'client_no_active_cases' => 'No active cases',
    'client_no_cases' => 'No cases at all',
    'appearance_mode' => 'Appearance',
    'color_theme' => 'Colour theme',
    'appearance_hint' => 'Saved to your account only — other users are unaffected.',
    'login_welcome' => 'Welcome to your legal workspace',
    'login_lead' => 'Sign in for secure access to your cases, documents and legal records.',
    'login_tagline' => 'An integrated legal workspace',
    'login_verifying' => 'Verifying your credentials…',
    'forgot_password_hint' => 'For security, password resets are handled by your office administrator — ask them to reset it for you from the Employees page.',
    'office_logo_alt' => ':office logo',

    // Layout / UI
    'switch_to_en' => 'English',
    'switch_to_ar' => 'العربية',
    'all_rights' => 'All rights reserved',
    'light_theme' => 'Light Theme',
    'dark_theme' => 'Dark Theme',
    'export_pdf' => 'Export PDF',
    'case_file' => 'Case File',
    'download_file' => 'Download File',
    'cases_files' => 'Case Files',

    // Page Titles
    'page_dashboard' => 'Dashboard',
    'page_cases' => 'Cases',
    'page_sessions' => 'Sessions',
    'page_tasks' => 'Tasks',
    'page_documents' => 'Documents',
    'page_clients' => 'Clients',
    'page_users' => 'Users',
    'page_settings' => 'Settings',
    'page_audit_log' => 'Audit Log',
    'page_notifications' => 'Notifications',
    'page_feasibility_study' => 'Feasibility Study',
    'page_profile' => 'Profile',
    'page_backup' => 'Backup',
    'page_add_case' => 'Add New Case',
    'page_edit_case' => 'Edit Case',
    'page_case_details' => 'Case Details',
    'page_add_session' => 'Add New Session',
    'page_edit_session' => 'Edit Session',
    'page_session_details' => 'Session Details',
    'page_add_task' => 'Add New Task',
    'page_edit_task' => 'Edit Task',
    'page_task_details' => 'Task Details',
    'page_add_client' => 'Add New Client',
    'page_edit_client' => 'Edit Client',
    'page_add_user' => 'Add New User',
    'page_edit_user' => 'Edit User',
    'page_trashed_cases' => 'Trashed Cases',
    'page_trashed_clients' => 'Trashed Clients',
    'page_my_cases' => 'My Cases',
    'page_my_sessions' => 'My Sessions',
    'page_my_documents' => 'My Documents',
    'page_my_case_details' => 'My Case Details',

    // Dashboard
    'total_cases' => 'Total Cases',
    'active_cases' => 'Active Cases',
    'overdue_cases' => 'Overdue Cases',
    'closed_cases' => 'Closed Cases',
    'cases_by_status' => 'Cases Distribution by Status',
    'upcoming_sessions' => 'Upcoming Sessions',
    'pending_tasks' => 'Pending Tasks',
    'recent_activity' => 'Recent Activity',
    'view_all' => 'View All',
    'no_upcoming_sessions' => 'No upcoming sessions',
    'no_pending_tasks' => 'No pending tasks',
    'no_recent_activity' => 'No recent activity',
    'no_due_date' => 'No due date',
    'case' => 'Case',

    // Client Portal
    'my_cases' => 'My Cases',
    'my_sessions' => 'My Sessions',
    'my_documents' => 'My Documents',
    'no_client_cases_msg' => 'No cases registered under your name',
    'no_client_sessions_msg' => 'No sessions scheduled for your cases',
    'no_client_documents_msg' => 'No documents available for you',
    'access_level' => 'Access Level',
    'public' => 'Public',
    'general' => 'General',

    // Sidebar
    'sidebar_home' => 'Main',
    'sidebar_cases' => 'Cases',
    'sidebar_sessions' => 'Sessions',
    'sidebar_tasks' => 'Tasks',
    'sidebar_documents' => 'Documents',
    'sidebar_clients' => 'Clients',
    'sidebar_users' => 'Users',
    'sidebar_feasibility' => 'Feasibility Study',
    'sidebar_audit' => 'Audit Log',
    'sidebar_settings' => 'Settings',
    'sidebar_profile' => 'Profile',
    'sidebar_logout' => 'Logout',

    // JS Confirm messages
    'confirm_action' => 'Are you sure?',
    'yes' => 'Yes',
    'no' => 'No',

    // Form placeholders
    'search_placeholder_general' => 'Search...',
    'filter_placeholder' => 'Filter...',

    // Status badge helper
    'badge_active' => 'Active',
    'badge_pending' => 'Pending',
    'badge_overdue' => 'Overdue',
    'badge_closed' => 'Closed',
    'badge_won' => 'Won',
    'badge_lost' => 'Lost',

    // Additional form fields
    'manual_input' => 'Manual Input',
    'handwriting_input' => 'Handwrite Entry',

    // Client inline creation
    'existing_client' => 'Select Existing Client',
    'new_client_inline' => '+ New Client',
    'save_client_add' => 'Save & Add Client',
    'saving' => 'Saving...',
    'name_required' => 'Name is required',
    'save_error' => 'Error saving',
    'connection_error' => 'Connection error',
    'ai_chat_title' => 'Smart Legal Assistant',
    'ai_chat_subtitle' => 'Omani law expert - answers about this case and legal questions',
    'ai_assistant_subtitle' => 'Legal questions and office cases only',
    'ai_assistant_greeting' => 'Welcome! I am the smart legal assistant. I only answer legal questions (per Omani law) and questions about the office\'s current cases.',
    'ai_assistant_placeholder' => 'Type your legal question here...',
    'ai_assistant_disclaimer' => 'Disclaimer: AI answers are for guidance only and do not replace professional legal advice; always verify against official legal texts.',
    'ai_chat_clear' => 'Clear conversation',
    'ai_chat_greeting' => 'Welcome! I am the smart legal assistant specialized in Omani laws. Ask me about this case or any legal matter (civil, labor, commercial, criminal...).',
    'ai_chat_placeholder' => 'Type your legal question here...',
    'ai_chat_send' => 'Send',
    'ai_chat_typing' => 'Typing...',
    'ai_chat_disclaimer' => 'Disclaimer: AI answers are for guidance only and do not replace professional legal advice; always verify against official legal texts.',
    'deleted_date' => 'Deletion Date',
    'user_info' => 'User Info',
    'document_singular' => 'document',
    'session_singular' => 'session',
    'task_singular' => 'task',
    'file_singular' => 'file',
    'model_placeholder' => 'e.g. User, Case',
    'role' => 'Role',
    'client_standalone' => 'Client',
    'lawyer_standalone' => 'Lawyer',
    'size' => 'Size',
    'uploaded' => 'Uploaded',
    'model' => 'Model',
    'ip_address' => 'IP Address',
    'items_per_page_label' => 'Items per page',
    'format' => 'Format',

    // Dashboard redesigned keys
    'win_rate' => 'Win Rate',
    'monthly_trend' => 'Monthly Trend',
    'office_overview' => 'Office Overview',
    'total_sessions' => 'Total Sessions',
    'last_session_date' => 'Last Session Date',
    'new_this_month' => 'New This Month',
    'from_last_month' => 'from last month',
    'today_sessions' => "Today's Sessions",
    'no_sessions_today' => 'No sessions today',
    'tasks_completed_week' => 'Tasks Completed This Week',
    'tasks_due_this_week' => 'tasks due this week',
    'no_tasks_due_this_week' => 'no tasks due this week',
    'total_tasks_all' => 'All tasks',
    'out_of' => 'out of',
    'cases_by_lawyer' => 'Cases by Lawyer',
    'cases_by_priority' => 'Cases by Priority',
    'overdue' => 'Overdue',
    'overdue_tasks' => 'Overdue Tasks',
    'no_overdue_tasks' => 'No overdue tasks',
    'document' => 'Document',
    'lawyers' => 'Lawyers',
    'no_data_available' => 'No data available',

    // Feasibility study redesigned keys
    'total_lawyers' => 'Total Lawyers',
    'office_win_rate' => 'Office Win Rate',
    'office_task_rate' => 'Office Task Rate',
    'team_average' => 'Team Average',
    'needs_improvement' => 'Needs Improvement',
    'efficiency_comparison' => 'Efficiency Comparison',
    'monthly_case_trends' => 'Monthly Case Trends',
    'cases_by_type' => 'Cases by Type',
    'team_comparison' => 'Team Comparison',
    'office_averages' => 'Office Averages',
    'workload_distribution' => 'Workload Distribution',
    'new_cases' => 'New Cases',
    'won' => 'Won',
    'lost' => 'Lost',
    'days' => 'days',
    'total_tasks' => 'Total Tasks',
    'closed' => 'Closed',
    'sessions' => 'Sessions',

    // Reports & Export
    'reports' => 'Reports',
    'reports_desc' => 'Export data as Excel files. Files open directly in Excel or Google Sheets.',
    'reports_hint' => 'Open the file in Excel or upload to Google Sheets via Import > File Upload',
    'record' => 'records',
    'export_excel' => 'Export Excel',
    'export_all' => 'Export All',

    // Evaluations
    'evaluations' => 'Performance Rating',
    'evaluations_desc' => 'Employee ratings are calculated automatically from case volume, interaction, and adherence to deadlines.',
    'evaluation_all' => 'All time',
    'evaluation_this_month' => 'This month',
    'evaluation_last_month' => 'Last month',
    'evaluation_rank' => 'Rank',
    'evaluation_employee' => 'Employee',
    'evaluation_score' => 'Score',
    'evaluation_cases' => 'Cases',
    'evaluation_sessions' => 'Sessions',
    'evaluation_tasks' => 'Tasks',
    'evaluation_documents' => 'Documents',
    'evaluation_activity' => 'Activity',
    'evaluation_closed' => 'closed',
    'evaluation_decisions' => 'decisions',
    'evaluation_on_time' => 'on time',
    'evaluation_cases_weight' => 'Case volume (40 pts)',
    'evaluation_cases_weight_desc' => '3 pts per closed/decided case, 1 pt per open case (max 40).',
    'evaluation_activity_weight' => 'Interaction & activity (35 pts)',
    'evaluation_activity_weight_desc' => 'Sessions, completed tasks, uploaded documents, and edit log (max 35).',
    'evaluation_quality_weight' => 'Quality & adherence (25 pts)',
    'evaluation_quality_weight_desc' => 'Task completion rate, on-time delivery, and case closure rate.',
    'grade_excellent' => 'Excellent',
    'grade_very_good' => 'Very Good',
    'grade_good' => 'Good',
    'grade_needs_improvement' => 'Needs Improvement',
    'no_data' => 'No data available',
    'export_all_desc' => 'Download all data (Cases + Sessions + Tasks + Clients) in one file',
    // HR & Finance
    'hr' => 'HR',
    'finance' => 'Finance',
    'hr_leave_type_annual' => 'Annual',
    'hr_leave_type_sick' => 'Sick',
    'hr_leave_type_emergency' => 'Emergency',
    'hr_leave_type_maternity' => 'Maternity',
    'hr_leave_type_unpaid' => 'Unpaid',
    'hr_leave_type_other' => 'Other',
    // Session warnings
    'session_warning_title' => 'Session Expiring',
    'session_warning_message' => 'Your session will expire automatically in',
    'continue' => 'Continue',
    'logout' => 'Log Out',
    'account_deactivated' => 'Your account has been deactivated.',
    // Add-case optional additions
    'add_optional' => 'Optional Additions',
    'add_optional_hint' => 'Add a document, task or note together with the case',
    'add_document' => 'Upload document',
    'document_title' => 'Document title',
    'document_file' => 'File',
    'access_all' => 'All',
    'add_task' => 'Add task',
    'task_due_date' => 'Due date',
    'add_note' => 'Add note',
    'note_title' => 'Note title',
    'note_content' => 'Note content',
    // Global UI
    'attention_center' => 'Attention Center',
    'admin_affairs' => 'Administrative Affairs',
    'developer_panel' => 'Developer Panel',
    'usage_guide' => 'Usage Guide',
    'monthly_cases' => 'Monthly Cases',
    'today_message' => "Today's Message",
    'product_credit' => 'Mudawala',
    'new_message' => 'New Message',
    'font_size' => 'Font Size',
    'font_increase' => 'Increase font size',
    'font_decrease' => 'Decrease font size',
    'font_reset' => 'Reset font size',
    // Cases UI
    'overdue_marked' => ':count newly overdue — the list now shows overdue cases only (:total).',
    'overdue_none' => 'No overdue cases found.',
    'overdue_existing' => 'Nothing new. The list shows overdue cases only (:total).',
    'delete_case_title' => 'Confirm case deletion',
    'delete_case_body' => 'Are you sure you want to delete case :case? This action cannot be undone.',
    'yes_delete' => 'Yes, Delete',
    'cancel' => 'Cancel',
    'send_followup' => 'Automatically send follow-up message to the client (email & WhatsApp)',
    // Monthly cases
    'monthly_cases_view' => 'Cases of',
    'month' => 'Month',
    'year' => 'Year',
    'loading' => 'Loading...',
    'total_cases' => 'Total Cases',
    'active' => 'Active',
    'pending' => 'Pending',
    'closed' => 'Closed',
    'subject' => 'Subject',
    'last_session_date' => 'Last Session Date',

    // أسماء يقرؤها قارئ الشاشة لأزرار لا نصّ ظاهر فيها
    'a11y_send_message' => 'Send message',
    'a11y_attach_file' => 'Attach a file',
    'a11y_cancel_reply' => 'Cancel reply',
    'a11y_remove_attachment' => 'Remove attachment',
    'a11y_message_text' => 'Message text',
    'a11y_toggle_brief' => 'Show or hide the brief',
    'a11y_test_connection' => 'Test the connection',
    'a11y_toggle_menu' => 'Show or hide the menu',
    'a11y_toggle_sidebar' => 'Collapse or expand the sidebar',
    'unauthorized_access' => 'You are not authorised to access this.',
    'scope' => 'Scope',
    'all_office_sessions' => 'All office sessions',
    'empty_evaluations_title' => 'No evaluation yet',
    'empty_evaluations_body' => 'Scores are computed from real work: cases, hearings, tasks and documents. Record some, then come back.',
    'empty_feasibility_title' => 'No figures for this period',
    'empty_feasibility_body' => 'Feasibility is computed from cases closed and fees recorded in the chosen period. Try a wider period, or close a case and record its fee.',
    'feas_intro_title' => 'What does this page tell you?',
    'feas_intro_body' => 'It measures the office from its own records: decided cases, completed tasks, and deadlines met. Every number below is explained — what it is computed from and what it means.',
    'feas_verdict' => 'Office summary',
    'feas_how' => 'How it is computed',
    'feas_how_win' => 'Cases won ÷ (won + lost). Cases not yet decided are excluded.',
    'feas_how_task' => 'Completed tasks ÷ all tasks.',
    'feas_how_deadline' => 'Tasks finished before their due date ÷ tasks that have one.',
    'feas_how_productivity' => 'Completed tasks ÷ active days. The target is two per day.',
    'feas_how_overall' => 'Weighted: wins 35% + task completion 25% + deadlines 25% + productivity 15%.',
    'feas_unit_tasks_day' => 'tasks/day',
    'feas_of_target' => 'of the :target tasks/day target',
    'feas_no_decided' => 'No decided case yet',
    'feas_no_decided_hint' => 'The win rate counts only won and lost cases. Close a case with an outcome to start the count.',
    'feas_verdict_strong' => 'The office is in good shape',
    'feas_verdict_mixed' => 'Mixed performance — see the detail',
    'feas_verdict_weak' => 'The office needs attention',
    'feas_verdict_body' => ':won of :decided decided cases were won, and :done of :tasks tasks completed.',
    'feas_team_table' => 'The team in detail',
    'feas_lawyer' => 'Lawyer',
    'feas_metric' => 'Metric',
    'feas_value' => 'Value',
    'feas_meaning' => 'Meaning',
    'suggestion_rate_limited' => 'You have sent ten suggestions in a short time. Wait :minutes minute(s) and try again.',
    'notif_task_assigned_title' => 'New task',
    'notif_task_assigned_body' => 'You have been assigned a new task: “:task”',
    'notif_task_done_title' => 'Task completed',
    'notif_task_done_body' => 'The task “:task” was completed',
    'notif_session_title' => 'New hearing',
    'notif_session_body' => 'A hearing was scheduled for case “:case” on :date',
    'notif_case_status_title' => 'Case status changed',
    'notif_case_status_body' => 'Case “:case” moved from :from to :to',
    'notif_leave_new_title' => 'New leave request',
    'notif_leave_new_body' => ':employee requested :type leave',
    'notif_leave_deduction_title' => 'Approved leave with deduction',
    'notif_leave_deduction_body' => ':employee · :days day(s) · expected deduction :amount OMR',
    'notif_leave_approved_title' => 'Leave approved',
    'notif_leave_approved_body' => 'Your :type leave request was approved',
    'notif_leave_rejected_title' => 'Leave rejected',
    'notif_leave_rejected_body' => 'Your :type leave request was rejected',
    'notif_suggestion_reply_title' => 'The developer replied to your suggestion',
    'suggestion_deleted' => 'Suggestion deleted',
    'suggestion_delete_denied' => 'You cannot delete someone else’s suggestion',
    'suggestion_delete_confirm' => 'Delete this suggestion?',
    'suggestion_delete' => 'Delete',
    'suggestion_state_reviewing' => 'Under review',
    'suggestion_state_planned' => 'Planned',
    'suggestion_state_declined' => 'Not planned',
    'suggestion_state_done' => 'Implemented',
    'notif_suggestion_done_title' => 'Your suggestion was implemented',
    'notif_suggestion_done_body' => 'Your suggestion “:excerpt” was implemented — thank you',
    'notif_reminder_title' => 'Reminder: :title',
    'notif_reminder_body' => 'Case: :case',
    'notif_chat_title' => ':sender sent you a message',
    'notif_passthrough' => ':text',
    'notif_auto_task_title' => 'New task (automatic): :task',
    'notif_auto_task_body' => 'From automation rule: :rule',
    'notif_auto_rule_title' => 'Automation rule: :rule',
    'notif_sub_expired_title' => 'The office subscription has ended',
    'notif_sub_expired_body' => 'The Mudawala subscription ended and the system is paused. Your data is fully intact — renew and the office is back at once.',
    'notif_sub_tomorrow_title' => 'The office subscription ends tomorrow',
    'notif_sub_tomorrow_body' => 'One day remains on the Mudawala subscription. Renew today to avoid an interruption.',
    'notif_sub_soon_title' => 'The office subscription ends in :days days',
    'notif_sub_soon_body' => 'The Mudawala subscription is nearing its end. Renewing now keeps the office running without a pause.',
    'notif_sub_early_body' => 'An early reminder of the Mudawala renewal date.',
    'notif_backup_remind_title' => 'Monthly reminder: your backup is healthy',
    'notif_backup_remind_body' => 'Last successful backup: :date (:size MB). It refreshes daily and is kept on the server and in a protected vault, and you can restore from it any time via Settings → Backup. We recommend downloading a copy to your own device now — a third location that you own.',
    'notif_backup_stale_title' => 'Warning: the backup is overdue',
    'notif_backup_stale_body' => 'The last successful backup was on :date — more than two days ago. Open Settings → Backup, create a manual copy now, and contact support if this keeps happening.',

    // WhatsApp — واتساب الأعمال
    // المفاتيح مسطّحة بلا تداخل، كبقيّة الملف: الواجهات تستدعيها __('app.wa_…')
    // مباشرةً، ولو جُمعت تحت مفتاحٍ أبٍ لأعاد __() المصفوفة نفسها فيطبع بليد
    // كلمة «Array» في مكان النص — عطبٌ صامت لا يُسقِط الصفحة فلا ينتبه له أحد.
    'page_whatsapp' => 'WhatsApp Business',
    'nav_whatsapp' => 'WhatsApp',
    // عائلة الشريط الجانبي تُسمّي عناصرها sidebar_*، فيُضاف المفتاح على
    // قياسها. ويبقى nav_whatsapp معه لأنّ layouts/app.blade.php ما يزال
    // يناديه — وحذفه اليوم يطبع «app.nav_whatsapp» خاماً في الشريط.
    'sidebar_whatsapp' => 'WhatsApp',

    // Inbox and conversation list
    'wa_inbox_title' => 'WhatsApp Inbox',
    'wa_no_conversations' => 'No conversations yet. The first message a client sends to the office number opens one here.',
    'wa_search_placeholder' => 'Search by contact name, number, or message text…',
    'wa_filter_all' => 'All',
    'wa_filter_unread' => 'Unread',
    'wa_filter_mine' => 'Assigned to me',
    'wa_filter_unassigned' => 'Unassigned',
    'wa_filter_open' => 'Open',
    'wa_filter_closed' => 'Closed',

    // Conversation row in the list
    // أكثرُ هذه المفاتيح بديلٌ يظهر حين تغيب البيانات — اسمٌ لم يصل من
    // ‏Meta، أو محادثةٌ بلا رسالةٍ بعد — أو عنوانٌ لأيقونةٍ لا نصَّ فيها.
    // بدونها يقرأ الموظّف شارةً صامتة لا يعرف معناها، ويقرأ قارئُ الشاشة
    // عدداً مجرّداً بلا سياق.
    'wa_unknown_contact' => 'Unknown contact',
    'wa_unread_aria' => 'Unread messages',
    'wa_preview_you' => 'You:',
    'wa_no_preview' => 'No messages yet',
    'wa_linked_client' => 'Linked to a client',
    'wa_linked_case' => 'Linked to a case',
    'wa_assigned_to' => 'Assigned to',
    'wa_no_conversations_hint' => 'When a client messages the office number on WhatsApp, the conversation opens here and the team is notified.',
    'wa_pick_conversation' => 'Pick a conversation',
    'wa_pick_conversation_hint' => 'Choose a conversation from the list to read it and reply.',

    // Connection state as the inbox sees it, not as the settings page does
    // الموظّف يفتح الصندوق ولا يملك الإعدادات: يُقال له ما الحال ومَن
    // يُصلحه، فلا يحسب الصفحة معطوبة ولا يُساق إلى بابٍ يردّه.
    'wa_not_connected_title' => 'No WhatsApp number connected yet',
    'wa_not_connected_hint' => 'The office manager connects the WhatsApp Business number once; after that, client messages arrive in this inbox and are answered from here.',
    'wa_not_connected_readonly' => 'The WhatsApp connection is off: earlier conversations can be read, but nothing goes out until it is connected again.',
    'wa_connect_now' => 'Connect now',
    'wa_ask_admin_to_connect' => 'Connecting is the office manager’s to do — ask them to finish it so conversations start arriving.',
    'wa_settings_link' => 'WhatsApp settings',
    'wa_number' => 'Office number',
    'wa_business_name' => 'WhatsApp business name',
    'wa_last_webhook' => 'Last event from Meta',

    // The 24-hour service window
    'wa_window_open' => 'Reply window open',
    'wa_window_closed' => 'Reply window closed',
    'wa_window_left' => ':minutes minute(s) left in the reply window',
    'wa_window_hint' => 'A free-form reply is allowed within twenty-four hours of the client’s last message. After that, only a Meta-approved template will reach them.',

    // Replying and internal notes
    'wa_send' => 'Send',
    'wa_reply_placeholder' => 'Write your reply…',
    'wa_internal_note' => 'Internal note',
    'wa_note_placeholder' => 'A note only the office team reads — it is never sent to the client…',
    'wa_note_saved' => 'Internal note saved',

    // The thread and the composer
    'wa_thread_title' => 'WhatsApp conversation',
    'wa_back_to_inbox' => 'Back to inbox',
    'wa_no_messages' => 'No messages in this conversation',
    'wa_no_messages_hint' => 'The first message received or sent will appear here.',
    'wa_tab_reply' => 'Reply',
    'wa_tab_note' => 'Note',
    'wa_save_note' => 'Save note',
    // الفرق بين ما يصل الموكّل وما لا يصله لونٌ في الواجهة، واللونُ وحده
    // لا يكفي لمن لا يميّزه: تصريحٌ نصيّ يمنع أن تُكتب ملاحظةٌ داخلية في
    // ظنّ صاحبها أنّها ردّ — أو أن يُكتب ردٌّ في ظنّ أنّه ملاحظة، وهو الأخطر.
    'wa_note_never_sent' => 'Internal note — never sent to the client',
    'wa_no_send_permission' => 'You do not have permission to send on WhatsApp — this view is read-only.',
    'wa_no_manage_permission' => 'You do not have permission to manage this conversation: linking, assigning and closing are for administrators.',

    // Outgoing message status
    'wa_status_queued' => 'Queued',
    'wa_status_sent' => 'Sent',
    'wa_status_delivered' => 'Delivered',
    'wa_status_read' => 'Read',
    'wa_status_failed' => 'Failed to send',

    // Attachments
    // ‏Meta لا تُرسل اسم ملفٍّ دائماً، فيحلّ نوعُ المرفق محلَّ الاسم —
    // وسطرٌ فارغ مكان الاسم يجعل المرفق السليم يبدو عطباً في العرض.
    'wa_media_image' => 'Image',
    'wa_media_document' => 'Document',
    'wa_media_audio' => 'Voice message',
    'wa_media_video' => 'Video',
    'wa_media_sticker' => 'Sticker',
    'wa_media_file' => 'Attachment',
    'wa_open_document' => 'Open document',
    'wa_link_case_first' => 'Link the conversation to a case to save the attachment into its file',
    // قد تردّ Meta برمز خطأ بلا وصف: يُقال إنّ الإرسال فشل ولم يُبيَّن
    // سببه، بدل سطرٍ خالٍ يوهم أنّ الرسالة وصلت.
    'wa_error_unknown' => 'Could not send — Meta gave no reason',

    // Linking, assigning, handing over
    'wa_link_client' => 'Link to client',
    'wa_link_case' => 'Link to case',
    'wa_unlinked' => 'Not linked',
    'wa_assign' => 'Assign conversation',
    'wa_unassigned' => 'Unassigned',
    'wa_handoff' => 'Hand over to a person',
    'wa_handoff_done' => 'Automatic replies stopped — the conversation is now with a person.',
    'wa_close' => 'Close conversation',
    'wa_reopen' => 'Reopen conversation',
    'wa_save_document' => 'Save to case documents',
    'wa_document_saved' => 'The attachment was saved to the case documents',
    'wa_open_case' => 'Open case',

    // Conversation panel: links, assignment, and the state of automatic replies
    'wa_links_title' => 'Links',
    'wa_link_client_first' => 'Link the conversation to a client first; their cases then appear here to choose from.',
    'wa_assign_to_me' => 'Assign to me',
    'wa_ai_active' => 'Automatic replies are running on this conversation inside the twenty-four hour window.',
    'wa_ai_stopped' => 'Automatic replies stopped',
    // التحويلُ والإغلاقُ يغيّران حال المحادثة عند الموكّل لا عندنا وحدنا،
    // وأحدهما لا رجعةَ عنه. فسؤالُ التأكيد قبلهما يمنع أن تُقلب محادثةٌ
    // حسّاسة بغلطةِ إصبعٍ على شاشة الهاتف.
    'wa_handoff_confirm' => 'Automatic replies stop on this conversation and do not resume. Hand it over to the team?',
    'wa_handoff_hint' => 'This stops automatic replies on this conversation only; no other conversation is affected.',
    'wa_close_confirm' => 'The conversation is closed and leaves the follow-up list; its messages are kept. Close it?',
    'wa_reopen_confirm' => 'The conversation returns to the follow-up list. Reopen it?',

    // Approved templates
    'wa_templates' => 'Templates',
    'wa_template_pick' => 'Pick an approved template',
    'wa_template_none_approved' => 'No approved template yet. Templates are approved in the Meta account, then synced here.',
    'wa_template_sync' => 'Sync templates',
    'wa_template_synced' => 'The template list was refreshed from Meta',
    // الخيار الفارغ في قائمةٍ إلزاميّة: نصٌّ يدلّ على أنّ الاختيار لم
    // يقع بعد، لا فراغٌ يُظنّ معه أنّ القائمة خالية من القوالب.
    'wa_template_none' => '— Pick a template —',
    'wa_template_variable' => 'Variable',

    // Connection settings
    'wa_settings_title' => 'WhatsApp Business connection',
    'wa_connected' => 'Connected',
    'wa_disconnected' => 'Not connected',
    'wa_needs_attention' => 'Needs attention',
    'wa_connect_help' => 'Connecting is a one-time job for the office manager: create an app in the Meta developer console, add the WhatsApp product to it, and link it to your WhatsApp Business Account and the office number. Then copy the permanent access token, the phone number ID, the WhatsApp Business Account ID and the app secret from the Meta console, and paste them into the fields below. Finally, take the webhook URL and verify token shown on this page, paste them into Meta’s Webhooks configuration, and subscribe to the messages field. Not one incoming message arrives until that last step is done.',
    "wa_connect_help_evolution" => "Link by scanning a code: press Start pairing, then open WhatsApp on the phone holding the office number → Settings → Linked devices → Link a device → point the camera at the code. The number you link becomes the office number in the system.",
    'wa_token' => 'Permanent access token',
    'wa_phone_id' => 'Phone Number ID',
    'wa_waba_id' => 'WhatsApp Business Account ID (WABA ID)',
    'wa_app_secret' => 'App Secret',
    'wa_app_id' => 'App ID',
    'wa_verify_token' => 'Verify token',
    'wa_webhook_url' => 'Webhook URL',
    'wa_copy' => 'Copy',
    'wa_copied' => 'Copied',
    'wa_test' => 'Test connection',
    'wa_test_ok' => 'The connection is healthy — the office number was read back from Meta.',
    'wa_test_failed' => 'Could not reach Meta. Check the entered credentials and that the token is still valid, then try again.',
    'wa_disconnect_confirm_title' => 'Disconnect WhatsApp?',
    'wa_disconnect_confirm_body' => 'The credentials are erased from this office and sending and receiving stop at once. Saved conversations and messages stay exactly as they are, and you can connect again at any time.',
    'wa_disconnect_yes' => 'Disconnect',
    'wa_cancel' => 'Cancel',

    // What is sent automatically
    'wa_notify_sessions' => 'Remind clients of hearing dates',
    'wa_notify_invoices' => 'Notify clients about invoices and dues',
    'wa_notify_case_updates' => 'Tell clients about updates to their cases',
    'wa_ai_reply' => 'Automatic replies to incoming messages',
    'wa_ai_reply_hint' => 'The assistant answers general questions inside the twenty-four hour window only. It does not go into case details and makes no promises on the office’s behalf, and the first request to speak with a person stops the automatic replies and hands the conversation to the team.',
    'wa_reminder_hours' => 'Remind this many hours before',
    'wa_opted_out' => 'This contact asked to stop receiving messages — nothing reaches them except a reply to a message they start.',
    'wa_pricing_note' => 'Meta bills per message sent, and from 1 October 2026 even free-form replies inside the twenty-four hour window become billable.',

    // In-app notifications
    'notif_wa_failed_title' => 'A WhatsApp message could not be sent',
    'notif_wa_failed_body' => 'The message did not reach the recipient. Reason: :reason',
    'notif_wa_handoff_title' => 'A WhatsApp conversation is waiting for a person',
    'notif_wa_handoff_body' => ':name asked to speak with a person — automatic replies on that conversation are stopped.',
    'notif_wa_incoming_title' => 'New WhatsApp message',
    'notif_wa_incoming_body' => 'A message arrived from :name',

    'task_status_changed' => 'Task is now :status.',
    'task_stage_hint_pending' => 'Work has not started yet.',
    'task_stage_hint_in_progress' => 'Work is under way.',
    'task_stage_hint_completed' => 'Done and closed.',
    'task_step_back' => 'Move back to :status',
    'task_reopen' => 'Reopen task',
    'task_completed_on' => 'Completed on :date',
    'task_action_in_progress' => 'Start work',
    'task_action_completed' => 'Mark it done',
    'ai_chat_copy' => 'Copy',
    'ai_chat_copied' => 'Copied',
    'ai_chat_retry' => 'Try again',
    'ai_chat_retrying' => 'Momentarily unreachable — retrying automatically…',
    'ai_chat_stop' => 'Stop',
    'ai_chat_clear_title' => 'Delete conversation',
    'ai_chat_clear_yes' => 'Delete',
    'ai_chat_cancel' => 'Cancel',
    'ai_chat_queued' => 'Your question is saved — the answer will appear here once the service is back.',
    'ai_chat_too_fast' => 'Too many questions in a short time. Wait a minute and send again.',
    'ai_chat_clear_confirm' => 'The whole conversation will be deleted and cannot be restored. Continue?',
    'ai_starter_1' => 'What are the steps to file a monetary claim in Oman?',
    'ai_starter_2' => 'What is the appeal window and how is it counted?',
    'ai_starter_3' => 'Summarise the status of the office\'s open cases.',
];
