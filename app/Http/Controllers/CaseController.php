...
class CaseController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            DB::rollBack();
            return redirect()->back()->withInput()->with('error', 'حدث خطأ أثناء ح�ظ القضية والجلسات: ' . $e->getMessage());
        }
    }
    ...
    public function monthly(Request $request): View
    {
        ...
        $months = [
            1 => 'يناير', 2 => '�براير', 3 => 'مارس', 4 => 'إبريل',
            5 => 'مايو', 6 => 'يونيو', 7 => 'يوليو', 8 => 'أغسطس',
            9 => 'سبتمبر', 10 => 'أكتوبر', 11 => 'نو�مبر', 12 => 'ديسمبر',
        ];
    ...
    public function analyze(LegalCase $case): JsonResponse
    {
        ...
        if (!$service->isConfigured()) {
            return response()->json([
                'error' => 'لم يتم إعداد م�تاح Gemini �ي مل� الإعدادات، يرجى التواصل مع المطور',
            ], 400);
        }
        ...
        $prompt = <<<PROMPT
أنت محام� خبير وأستاذ قانون �ي سلطنة عمان، متخصص �ي تطبيق القوانين العمانية السارية:
- قانون المعاملات المدنية العماني
- قانون الإجراءات المدنية والتجارية العماني
- قانون الإثبات �ي المعاملات المدنية والتجارية العماني
- قانون العمل العماني
- قانون الشركات التجارية العماني
- قانون التجارة العماني
- قانون الجزاء العماني وقانون الإجراءات الجزائية العماني
- قانون المرا�عات الشرعية العماني وأحكام الأحوال الشخصية
- قوانين التن�يذ المدني والجزائي العماني
- نظام المحاماة العماني وقرارات المهن القانونية
- أحكام المحكمة العليا العمانية ومبادئها المستقرة

قم بتحليل القضية التالية بشكل احترا�ي وعميق باللغة العربية:
...
1. **تقييم القضية**: تحليل قوة الدعوى و�رص نجاحها و�قاً للقانون العماني، مع ذكر الأساس القانوني والمبادئ المستند إليها.
...
3. **نقاط ضع� الخصم**: تحليل نقاط الضع� �ي موق� الخصم والد�وع المحتملة ضده.
...
6. **المخاطر**: مخاطر محتملة وكي�ية التعامل معها.

تعليمات الأسلوب والعمق:
- تصر� وكأنك أشطر محام� وأخبر خبير قانوني �ي العالم، وكأنك تقدم مرا�عة مكتوبة لمحكمة عليا.
- كن موسعاً وشاملاً: لا تختصر، ولا تنتقل بين الأقسام بسرعة، وا�تح كل قسم بشرح متعمق وت�صيل دقيق واستدلال قانوني قوي.
...
- كن واقعياً ومحايداً ولا تبالغ �ي التوقعات.
- إذا كانت المعلومات غير كا�ية �ي بعض النقاط، اذكر ذلك بصراحة واقترح ما يجب استكماله.
- التزم بالقوانين السارية �ي سلطنة عمان �قط.
...
PROMPT;
    ...
    public function aiChat(Request $request, LegalCase $case): JsonResponse
    {
        ...
        $systemPrompt = <<<SYSTEM
أنت مساعد قانوني ذكي مدمج �ي نظام إدارة مكتب محاماة عماني. أنت محام� خبير �ي القوانين السارية �ي سلطنة عمان (قانون المعاملات المدنية، قانون الإجراءات المدنية والتجارية، قانون الإثبات، قانون العمل، قانون الشركات التجارية، قانون التجارة، قانون الجزاء، قانون الإجراءات الجزائية، قانون المرا�عات الشرعية، قوانين التن�يذ، نظام المحاماة، وأحكام المحكمة العليا العمانية).
...
قواعد الرد:
- أجب باللغة العربية ال�صحى دائماً.
- استند �ي إجاباتك إلى القانون العماني �قط، واذكر القانون أو المبدأ ذي الصلة.
...
- إذا س�ئلت عن شيء خارج القانون أو خطر، اعتذر بلط�.
SYSTEM;
    ...
    private function notifyClientPortal(LegalCase $case): array
    {
        ...
        if ($client->email) {
            if (config('mail.default', 'log') !== 'log') {
                ...
                $failures[] = 'الإيميل غير م�عل �ي إعدادات الخادم';
            }
        }
    ...
    {
        return <<<TXT
يسر **شركة حمد الريامي للمحاماة (شركة مدنية للمحاماة)** أن تضع بين أيديكم خدمة **متابعة القضايا إلكترونياً**، وذلك حرصاً منا على تعزيز جودة الخدمات القانونية، وتو�ير تجربة أكثر سهولة وش�ا�ية لموكلينا الكرام.

يمكنكم الاطلاع على آخر مستجدات القضية، ومتابعة ت�اصيلها بكل يسر، من خلال الدخول إلى الرابط التالي:

https://office.riyami.om/client-access

بعد �تح الرابط، ي�رجى إدخال **رقم الهات�** أو **البريد الإلكتروني** المسجل لدى المكتب، لتظهر لكم جميع ت�اصيل القضية والمستجدات المتعلقة بها بشكل مباشر.

و�ي حال واجهتكم أي صعوبة �ي الدخول أو كانت لديكم أي است�سارات، �إن �ريقنا على أتم الاستعداد لخدمتكم والإجابة عن جميع است�ساراتكم.

**شركة حمد الريامي للمحاماة (شركة مدنية للمحاماة)**
نعتز بثقتكم، ونسعى دائماً إلى تقديم خدمات قانونية احترا�ية بأعلى معايير الجودة.
TXT;
    }
