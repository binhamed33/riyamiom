    ...
    private function sendTest(string $to): bool
    {
        ...
        try {
            if ($this->option('now')) {
                // sendNow لا send.
                //
                // ═══ ولماذا هذا فرقٌ لا تفصيل ═══
                //
                // كلُّ رسائل النظام ترث OfficeMail، وهي ShouldQueue.
                // و Mail::to()->send() تفحص ذلك: Mailer::sendMailable()
                // تُرجع ‎->queue() للمؤجَّلة و‎->send() لغيرها. فكان
                // ‎--now يُلقي الرسالة في الطابور ثم يطبع «أُرسلت
                // مباشرةً» ويخرج بصفر — ولم يُفتح اتصالُ SMTP قطّ،
                // فلم يكن للـ catch أدناه ما يمسكه أصلاً.
                //
                // وهذا يعني أنّ «جرّبها بـ ‎--now» — وهي الخطوة التي
                // تختم بها أداةُ ضبط كلمة المرور وملفُّ MAIL.md — كانت
                // تقول «نجح» عن كلمة مرورٍ مرفوضة، وتترك ٥٣٥ يتراكم في
                // failed_jobs بعد ربع ساعة حيث لا ينظر أحد.
