...
class WhatsAppSweep extends Command
{
    ...
    protected function revivePairing(): void
    {
        ...
        if ($state === 'open') {
            $this->line('  أُعيد وصلُ الرقم تلقائياً.');

            return;
        }

        // ═══ ولا يُترك المكتبُ أمام بابٍ مسدود ═══
        //
        // نسخةٌ غادرت ذاكرةَ الجسر وصفُّها باقٍ في قاعدته: «connect»
        // يقول «غير موجودة» إلى الأبد، فلا إحياءَ ولا رمزَ حين يفتح
        // المكتبُ الإعدادات — يقرأ خطأً لا مخرجَ منه.
        //
        // فيُحضَّر الطريقُ هنا: pair() يحذف الميتَ وينشئ نظيفاً، فيجد
        // المكتبُ رمزاً حاضراً متى فتح الصفحة. ولا تُلمس جلسةٌ حيّة:
        // الحالةُ ليست open ولا connecting، والفصلُ الصريحُ فُحص فوق.
        try {
            $pair = (new \App\Services\WhatsApp\EvolutionProvider())->pair();
        } catch (\Throwable $e) {
            $this->line('  تعذّر تحضير الاقتران: ' . $e->getMessage());

            return;
        }

        $this->line(match (true) {
            $pair['state'] === 'open' => '  أُعيد وصلُ الرقم تلقائياً.',
            $pair['qr'] !== null => '  الاقتران ' . $state . ' — رمزٌ جاهزٌ في الإعدادات متى فتحها المكتب.',
            default => '  الاقتران ما زال ' . $state . ' — ' . ($pair['message'] ?? 'الهاتف يحتاج مسحَ الرمز.'),
        });
    }
