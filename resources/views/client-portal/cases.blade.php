...
<div class="space-y-6" dir="rtl">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-[#C9A55A]">{{ __('app.my_cases') }}</h1>
    </div>
            ...
            <table class="w-full text-sm text-right">
                <thead>
                    <tr class="border-b border-white/10">
                        ...
                        <th class="px-4 py-3 text-[#C9A55A] font-bold whitespace-nowrap">{{ __('app.case_next_date') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                    @forelse($cases as $case)
                        <tr class="hover:bg-white/5 transition-colors cursor-pointer" onclick="window.location='{{ route('client.cases.show', $case) }}'">
