...
@section('header-actions')
    <button onclick="history.back()" class="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm rounded-xl bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors border border-amber-100">
...
<div class="animate-fade-in">
        ...
        <div class="relative">
            <div class="flex items-center gap-2 sm:gap-3 mb-2">
                ...
                <div>
                    ...
                    <p class="text-amber-200/80 text-xs sm:text-sm mt-0.5">رقم القضية: #{{ $case->office_case_number ?? $case->case_number }}</p>
                </div>
            </div>
            <div class="flex flex-wrap items-center gap-2 mt-3">
                <span class="text-[10px] sm:text-xs px-2 py-0.5 sm:px-3 sm:py-1 rounded-lg font-medium
                    ...
                    {{ $case->status === 'pending' ? 'bg-amber-500/20 text-amber-200' : '' }}
                ...
                <span class="text-[10px] sm:text-xs text-amber-200/60">{{ $case->case_type ?? $case->type }}</span>
                <span class="text-amber-200/40">·</span>
                <span class="text-[10px] sm:text-xs text-amber-200/60">{{ $case->court }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
            {{-- Case Details Card --}}
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-amber-100 p-5 sm:p-7 shadow-sm">
                <h2 class="text-xs sm:text-sm font-bold text-amber-700 uppercase tracking-wider mb-4 sm:mb-5 font-heading flex items-center gap-2">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            @if($case->description)
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-amber-100 p-5 sm:p-7 shadow-sm">
                <h2 class="text-xs sm:text-sm font-bold text-amber-700 uppercase tracking-wider mb-3 font-heading flex items-center gap-2">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-amber-100 shadow-sm overflow-hidden">
                <div class="px-5 sm:px-7 py-4 sm:py-5 border-b border-amber-50">
                    <h2 class="text-xs sm:text-sm font-bold text-amber-700 uppercase tracking-wider font-heading flex items-center gap-2">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                ...
                <div class="divide-y divide-amber-50">
                    @foreach($pastSessions as $session)
                        <div class="px-5 sm:px-7 py-4 sm:py-5 hover:bg-amber-50/30 transition-colors">
                            ...
                            @if($session->report)
                                <div class="mt-2">
                                    ...
                                    <p class="text-xs sm:text-sm text-gray-700 bg-amber-50 rounded-xl p-3 sm:p-4 leading-relaxed border border-amber-100/50">{{ $session->report }}</p>
                                </div>
                            @endif
                        </div>
                    @endforeach
        ...
        <div class="space-y-4 sm:space-y-6">
            ...
            {{-- Quick Info --}}
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-amber-100 p-5 sm:p-7 shadow-sm">
                <h2 class="text-xs sm:text-sm font-bold text-amber-700 uppercase tracking-wider mb-4 font-heading flex items-center gap-2">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
