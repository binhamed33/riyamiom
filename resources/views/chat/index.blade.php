...
<div class="flex gap-4 h-[calc(100vh-10rem)]">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400 truncate mt-0.5">
                                @if($conv->lastMessage)
                                    ...
                                    @endif
                                @else
                                    <span class="text-gray-300">لا توجد رسائل</span>
                                @endif
                            </p>
                        </div>
                    </div>
        ...
        @else
            ...
            <div class="flex-1 flex items-center justify-center">
                <div class="text-center">
                    <svg class="w-20 h-20 text-gray-100 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    <h3 class="text-gray-400 font-medium text-lg">{{ __('app.chat') ?? 'المحادثات' }}</h3>
                    <p class="text-gray-300 text-sm mt-1">اختر محادثة أو ابدأ محادثة جديدة مع موظف</p>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="w-72 flex-shrink-0 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            @empty
                <div class="p-8 text-center text-gray-400 text-sm">لا يوجد موظفون آخرون</div>
            @endforelse
        </div>
    </div>
</div>
        ...
        editForm.querySelector('.edit-save').addEventListener('click', async function() {
            ...
            .then(data => {
                if (textEl) {
                    ...
                    editedSpan.className = 'text-[10px] text-gray-400';
