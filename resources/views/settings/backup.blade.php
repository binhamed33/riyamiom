...
<div class="space-y-6" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <form action="{{ route('backup.create') }}" method="POST">
            @csrf
            <button type="submit" class="bg-primary hover:bg-primary-dark text-white px-6 py-2.5 rounded-lg font-semibold transition-colors text-sm inline-flex items-center gap-2">
    ...
    {{-- Security Warning --}}
    <div class="bg-yellow-100 border border-yellow-200 rounded-xl p-4">
        <div class="flex items-start gap-3">
            <svg class="w-5 h-5 text-yellow-700 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
            </svg>
            <div>
                <p class="text-yellow-700 text-sm font-bold">{{ __('app.backup_important_title') }}</p>
                <p class="text-yellow-700 text-xs mt-1">{{ __('app.backup_important_text') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gold/15 p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-gray-900 text-sm font-bold">{{ __('app.auto_backup_enabled') }}</p>
                <p class="text-gray-400 text-xs mt-0.5">{{ __('app.auto_backup_schedule') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gold/15 overflow-hidden">
        @if(!empty($backups) && count($backups) > 0)
            <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                ...
                <span class="text-gray-300 text-xs">{{ __('app.backup_auto_keep_30') }}</span>
            </div>
                    ...
                    <tbody>
                        @foreach($backups as $backup)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3 text-gray-400 text-xs">{{ $backup['size'] }} MB</td>
                                <td class="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{{ $backup['date'] }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('backup.download', $backup['name']) }}"
                                           class="px-3 py-1.5 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded-lg text-xs font-medium transition-colors inline-flex items-center gap-1">
                                        ...
                                        <form action="{{ route('backup.destroy', $backup['name']) }}" method="POST" class="contents" x-data @submit.prevent="if(confirm('{{ __("app.confirm_delete_backup") }}')) $el.submit()">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="text-center py-12 text-gray-500">
                ...
                <p class="text-xs text-gray-600 mt-1">{{ __('app.create_first_backup') }}</p>
            </div>
        @endif
    </div>
