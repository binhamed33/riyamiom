...
<div class="">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-amber-200 p-5">
            ...
            <p class="text-2xl font-bold text-amber-700">{{ number_format($stats['balance'], 2) }} ر.ع</p>
        </div>
        <div class="bg-white rounded-xl border border-amber-200 p-5">
    ...
    <div class="mb-6 border-b border-gray-200 flex gap-1 overflow-x-auto">
        <a href="{{ route('finance.index', ['tab' => 'transactions']) }}" class="px-5 py-3 text-sm font-medium transition rounded-t-lg whitespace-nowrap {{ $tab === 'transactions' ? 'text-amber-700 bg-gray-100 border-b-2 border-amber-700' : 'text-gray-400 hover:text-gray-600' }}">المعاملات</a>
        <a href="{{ route('finance.index', ['tab' => 'invoices']) }}" class="px-5 py-3 text-sm font-medium transition rounded-t-lg whitespace-nowrap {{ $tab === 'invoices' ? 'text-amber-700 bg-gray-100 border-b-2 border-amber-700' : 'text-gray-400 hover:text-gray-600' }}">الفواتير</a>
        <a href="{{ route('finance.index', ['tab' => 'fees']) }}" class="px-5 py-3 text-sm font-medium transition rounded-t-lg whitespace-nowrap {{ $tab === 'fees' ? 'text-amber-700 bg-gray-100 border-b-2 border-amber-700' : 'text-gray-400 hover:text-gray-600' }}">رسوم القضايا</a>
    </div>
        ...
        <div class="bg-white rounded-xl border border-amber-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead><tr class="border-b border-gray-200"><th class="text-right px-4 py-3 font-bold text-amber-700">النوع</th><th class="text-right px-4 py-3 font-bold text-amber-700">التصنيف</th><th class="text-right px-4 py-3 font-bold text-amber-700">المبلغ</th><th class="text-right px-4 py-3 font-bold text-amber-700">التاريخ</th><th class="text-right px-4 py-3 font-bold text-amber-700">طريقة الدفع</th><th class="text-center px-4 py-3 font-bold text-amber-700">إجراءات</th></tr></thead>
                    <tbody>
    ...
    @elseif($tab === 'invoices')
        <div class="bg-white rounded-xl border border-amber-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead><tr class="border-b border-gray-200"><th class="text-right px-4 py-3 font-bold text-amber-700">رقم الفاتورة</th><th class="text-right px-4 py-3 font-bold text-amber-700">العميل</th><th class="text-right px-4 py-3 font-bold text-amber-700">المبلغ</th><th class="text-right px-4 py-3 font-bold text-amber-700">المتبقي</th><th class="text-right px-4 py-3 font-bold text-amber-700">الحالة</th><th class="text-right px-4 py-3 font-bold text-amber-700">تاريخ الإصدار</th><th class="text-center px-4 py-3 font-bold text-amber-700">إجراءات</th></tr></thead>
                    <tbody>
                        @forelse($invoices as $inv)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('finance.invoices.show', $inv) }}" class="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        @if($isFinAdmin)<button @click="$dispatch('open-modal', 'invEditModal'); $dispatch('set-inv', {!! $inv->toJson() !!})" class="w-8 h-8 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>@endif
                                        ...
                                        @if($isFinAdmin)<form method="POST" action="{{ route('finance.invoices.destroy', $inv) }}" onsubmit="return confirm('حذف الفاتورة؟')" class="inline">@csrf @method('DELETE')<button class="w-8 h-8 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>@endif
                                    </div>
                                </td>
                            </tr>
                        @empty
    ...
    @elseif($tab === 'fees')
        <div class="bg-white rounded-xl border border-amber-200 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead><tr class="border-b border-gray-200"><th class="text-right px-4 py-3 font-bold text-amber-700">القضية</th><th class="text-right px-4 py-3 font-bold text-amber-700">نوع الرسم</th><th class="text-right px-4 py-3 font-bold text-amber-700">المبلغ</th><th class="text-right px-4 py-3 font-bold text-amber-700">الحالة</th><th class="text-right px-4 py-3 font-bold text-amber-700">التاريخ</th><th class="text-center px-4 py-3 font-bold text-amber-700">إجراءات</th></tr></thead>
                    <tbody>
                        @forelse($fees as $fee)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-1">
                                        <a href="{{ route('finance.fees.show', $fee) }}" class="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                                        @if($isFinAdmin)<button @click="$dispatch('open-modal', 'feeEditModal'); $dispatch('set-fee', {!! $fee->toJson() !!})" class="w-8 h-8 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>@endif
                                        @if($isFinAdmin)<form method="POST" action="{{ route('finance.fees.destroy', $fee) }}" onsubmit="return confirm('حذف الرسم؟')" class="inline">@csrf @method('DELETE')<button class="w-8 h-8 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors flex items-center justify-center"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>@endif
                                    </div>
                                </td>
                            </tr>
...
<div id="txModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data="{ open: false }" x-show="open" x-cloak x-on:open-modal.window="if ($event.detail === 'txModal') open = true" @keydown.escape="open = false">
    ...
    <div class="relative bg-white border border-amber-300 rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col" x-on:click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            ...
            <button @click="open = false" class="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-900 transition-colors"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
        <form method="POST" action="{{ route('finance.transactions.store') }}" enctype="multipart/form-data" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required><option value="income">دخل</option><option value="expense">مصروف</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">التصنيف <span class="text-red-700">*</span></label>
                    <input type="text" name="category" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="payment_method" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المرجع</label>
                    <input type="text" name="reference" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
            </div>
            <div>
        ...
        <form method="POST" action="" x-bind:action="'/finance/transactions/' + tx.id" enctype="multipart/form-data" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required x-model="tx.type"><option value="income">دخل</option><option value="expense">مصروف</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">التصنيف</label>
                    <input type="text" name="category" x-model="tx.category" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المبلغ</label>
                    <input type="number" step="0.001" name="amount" x-model="tx.amount" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">التاريخ</label>
                    <input type="date" name="date" x-model="tx.date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">طريقة الدفع</label>
                    <input type="text" name="payment_method" x-model="tx.payment_method" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المرجع</label>
                    <input type="text" name="reference" x-model="tx.reference" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">الوصف</label>
                <textarea name="description" rows="2" x-model="tx.description" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"></textarea>
            </div>
            <div>
        ...
        <form method="POST" action="{{ route('finance.invoices.store') }}" enctype="multipart/form-data" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="invoice_number" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">العميل</label>
                    <select name="client_id" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"><option value="">اختر</option>@foreach($clients as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach</select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المبلغ <span class="text-red-700">*</span></label>
                    <input type="number" step="0.001" name="amount" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                ...
                <div>
                    ...
                    <select name="status" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required><option value="unpaid">غير مدفوعة</option><option value="paid">مدفوعة</option><option value="partial">مدفوعة جزئياً</option><option value="cancelled">ملغية</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">تاريخ الإصدار <span class="text-red-700">*</span></label>
                    <input type="date" name="issue_date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">تاريخ الاستحقاق</label>
                    <input type="date" name="due_date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">الوصف</label>
        ...
        <form method="POST" action="" x-bind:action="'/finance/invoices/' + inv.id" enctype="multipart/form-data" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="invoice_number" x-model="inv.invoice_number" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المبلغ</label>
                    <input type="number" step="0.001" name="amount" x-model="inv.amount" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">الحالة</label>
                    <select name="status" x-model="inv.status" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"><option value="unpaid">غير مدفوعة</option><option value="paid">مدفوعة</option><option value="partial">مدفوعة جزئياً</option><option value="cancelled">ملغية</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">تاريخ الإصدار</label>
                    <input type="date" name="issue_date" x-model="inv.issue_date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">تاريخ الاستحقاق</label>
                    <input type="date" name="due_date" x-model="inv.due_date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">الوصف</label>
                <textarea name="description" rows="2" x-model="inv.description" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">تغيير المرفق</label>
                <input type="file" name="attachment" class="w-full text-gray-500 text-sm file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-100 file:text-amber-600">
        ...
        <form method="POST" action="{{ route('finance.fees.store') }}" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    ...
                    <select name="case_id" class="ts w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required><option value="">اختر</option>@foreach($cases as $case)<option value="{{ $case->id }}">#{{ $case->office_case_number }} - {{ $case->case_number ?? '' }} - {{ $case->client?->phone ?? '' }} - {{ $case->client?->name ?? '' }}</option>@endforeach</select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">نوع الرسم <span class="text-red-700">*</span></label>
                    <input type="text" name="fee_type" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المبلغ <span class="text-red-700">*</span></label>
                    <input type="number" step="0.001" name="amount" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">الحالة <span class="text-red-700">*</span></label>
                    <select name="status" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required><option value="unpaid">غير مدفوع</option><option value="paid">مدفوع</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">التاريخ <span class="text-red-700">*</span></label>
                    <input type="date" name="date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
        ...
        <form method="POST" action="" x-bind:action="'/finance/fees/' + fee.id" class="px-6 py-5 overflow-y-auto space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    ...
                    <select name="case_id" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"><option value="">اختر</option>@foreach($cases as $case)<option value="{{ $case->id }}" x-bind:selected="fee.case_id == {{ $case->id }}">#{{ $case->office_case_number }} - {{ $case->case_number ?? '' }} - {{ $case->client?->phone ?? '' }} - {{ $case->client?->name ?? '' }}</option>@endforeach</select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">نوع الرسم</label>
                    <input type="text" name="fee_type" x-model="fee.fee_type" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">المبلغ</label>
                    <input type="number" step="0.001" name="amount" x-model="fee.amount" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">الحالة</label>
                    <select name="status" x-model="fee.status" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500"><option value="unpaid">غير مدفوع</option><option value="paid">مدفوع</option></select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-1.5">التاريخ</label>
                    <input type="date" name="date" x-model="fee.date" class="w-full rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-900 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500" required>
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1.5">الوصف</label>
