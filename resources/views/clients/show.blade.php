...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-amber-600">{{ $client->name }}</h1>
        <div class="flex items-center gap-3">
            <a href="{{ route('clients.edit', $client) }}" class="bg-amber-500 hover:bg-amber-600 text-white px-6 py-2.5 rounded-lg font-semibold transition-colors text-sm">{{ __('app.edit') }}</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <h2 class="text-lg font-semibold text-amber-600 mb-4">{{ __('app.client_details') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-white rounded-lg p-4">
                        ...
                        @if($client->type === 'company')
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-600 border border-amber-300">{{ __('app.company') }}</span>
                        @else
        ...
        <div>
            <div class="bg-white rounded-xl border border-amber-200 p-6">
                <div class="text-center">
                    <p class="text-5xl font-bold text-amber-600">{{ $client->cases_count ?? 0 }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-amber-600">{{ __('app.client_cases') }}</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200">
                        <th class="text-right px-6 py-3 text-sm font-semibold text-amber-600">{{ __('app.case_number') }}</th>
                        <th class="text-right px-6 py-3 text-sm font-semibold text-amber-600">{{ __('app.title') }}</th>
                        <th class="text-right px-6 py-3 text-sm font-semibold text-amber-600">{{ __('app.status') }}</th>
                        <th class="text-right px-6 py-3 text-sm font-semibold text-amber-600">{{ __('app.case_lawyer') }}</th>
                        <th class="text-right px-6 py-3 text-sm font-semibold text-amber-600">{{ __('app.created_at') }}</th>
                    </tr>
                </thead>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ $case->status === 'active' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                                       ($case->status === 'closed' ? 'bg-gray-100 text-gray-500 border border-gray-200' : 'bg-amber-100 text-amber-700 border border-amber-200') }}">
                                    {{ $case->status }}
                                </span>
                            </td>
