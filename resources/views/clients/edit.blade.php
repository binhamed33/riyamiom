...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gold">{{ __('app.page_edit_client') }}</h1>
        <a href="{{ route('clients.index') }}" class="text-ivory/50 hover:text-ivory transition">{{ __('app.back_to_list') }}</a>
    </div>

    <form method="POST" action="{{ route('clients.update', $client) }}" class="bg-navy-light rounded-xl border border-ivory/10 p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="individual" {{ old('type', $client->type) === 'individual' ? 'checked' : '' }} class="w-4 h-4 text-gold bg-navy border-ivory/30 focus:ring-gold" required>
                    <span class="text-ivory">{{ __('app.individual') }}</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="company" {{ old('type', $client->type) === 'company' ? 'checked' : '' }} class="w-4 h-4 text-gold bg-navy border-ivory/30 focus:ring-gold">
