...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gold">{{ __('app.page_clients') }}</h1>
        <a href="{{ route('clients.create') }}" class="bg-gold hover:bg-gold-dark text-navy px-6 py-2.5 rounded-lg font-semibold transition-colors text-sm">
    ...
    <form method="GET" action="{{ route('clients.index') }}">
        <div class="flex items-center gap-3">
            ...
            >
            <button type="submit" class="bg-gold hover:bg-gold-dark text-navy px-6 py-2.5 rounded-lg font-semibold transition-colors text-sm">{{ __('app.search') }}</button>
            @if(request('search'))
                <a href="{{ route('clients.index') }}" class="text-ivory/50 hover:text-ivory transition px-3 py-2.5">{{ __('app.clear') }}</a>
            @endif
        </div>
    </form>
                ...
                <tbody class="divide-y divide-ivory/5">
                    @forelse($clients as $client)
                        <tr class="hover:bg-navy-lighter/50 transition">
                            <td class="px-6 py-4">
                                <a href="{{ route('clients.show', $client) }}" class="text-ivory hover:text-gold transition font-medium">
                                    {{ $client->name }}
                                </a>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('clients.edit', $client) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#C9A55A]/10 text-[#C9A55A] hover:bg-[#C9A55A]/20 transition-colors" title="{{ __('app.edit') }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-ivory/30">{{ __('app.no_clients') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
