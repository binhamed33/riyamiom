...
<div class="space-y-6" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 class="text-2xl font-bold text-[#C9A55A]">{{ __('app.page_trashed_cases') }}</h1>
        <a href="{{ route('cases.index') }}" class="bg-white/10 hover:bg-white/20 text-white/70 px-6 py-2.5 rounded-lg font-medium transition-colors text-sm flex items-center gap-2">
            ...
            <table class="w-full text-sm text-right">
                <thead>
                    <tr class="border-b border-white/10">
                        ...
                        <th class="px-4 py-3 text-[#C9A55A] font-bold whitespace-nowrap">{{ __('app.case_client') }}</th>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($cases as $case)
                        <tr class="hover:bg-white/5 transition-colors">
                            ...
                            <td class="px-4 py-3 text-white/30 text-xs">{{ $case->client->name ?? '—' }}</td>
                            ...
                            <td class="px-4 py-3">
                                <form action="{{ route('cases.restore', $case->id) }}" method="POST" class="contents" onsubmit="return confirm('{{ __('app.restore_case_confirm') }}')">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition-colors" title="{{ __('app.restore') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
