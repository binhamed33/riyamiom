...
@section('title', 'إعدادات الاشتراك')
...
<div class="max-w-3xl mx-auto space-y-6" dir="rtl" x-data="{
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gold-dark flex items-center gap-2">إعدادات الاشتراك</h1>
            <p class="text-sm text-gray-500 mt-1">اشتراك هذه النسخة من النظام — صلاحية المطور فقط</p>
        </div>
        <div class="text-xs text-gray-400">وقت السيرفر: <span dir="ltr">{{ now()->format('d M Y H:i:s') }}</span></div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="px-6 sm:px-8 py-6 border-b border-gray-100 flex items-center justify-between">
            <span class="text-xs font-bold text-gray-400">حالة اشتراك النظام</span>
        ...
        <div class="px-6 sm:px-8 py-8">
            @if($noneOrExpired)
                <div class="text-center py-6">
                    ...
                    <p class="text-sm text-gray-500 mb-6">اختر مدة الاشتراك ثم اضغط «تفعيل الاشتراك». سيبدأ الحساب من وقت السيرفر الحالي.</p>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-8">
                    ...
                    <div class="sm:w-64 bg-gradient-to-br from-charcoal to-navy text-white rounded-2xl p-6 text-center" style="background:linear-gradient(150deg,#121826,#0D111B);">
                        ...
                        <p class="text-[11px] text-gray-400 font-mono" dir="ltr" x-data="{
                            ...
                            <span x-text="Math.floor(this.remaining / 86400) + ' يوم ' + Math.floor(this.remaining % 86400 / 3600) + ' سا ' + Math.floor(this.remaining % 3600 / 60) + ' د ' + (this.remaining % 60) + ' ث'"></span>
                        </p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 text-sm text-gray-500 leading-relaxed">
        <p class="font-semibold text-gray-700 mb-1">مصدر الحقيقة هو وقت السيرفر</p>
    ...
    <div id="activateModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800">تفعيل الاشتراك</h3>
                <button type="button" @click="closeModal('activateModal')" aria-label="{{ __('app.close') }}" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
            <form method="POST" action="{{ route('developer.subscription.activate') }}" class="space-y-5">
                ...
                <button type="submit" class="w-full bg-gold hover:bg-gold-dark text-white px-5 py-3 rounded-xl font-bold transition-colors">تفعيل الاشتراك</button>
            </form>
        </div>
    </div>
    ...
    <div id="changeModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800">تغيير الاشتراك</h3>
                <button type="button" @click="closeModal('changeModal')" aria-label="{{ __('app.close') }}" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
    ...
    <div id="reactivateModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-bold text-gray-800">إعادة تفعيل الاشتراك</h3>
                <button type="button" @click="closeModal('reactivateModal')" aria-label="{{ __('app.close') }}" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
            <p class="text-sm text-gray-500 mb-4">
            ...
            <form method="POST" action="{{ route('developer.subscription.reactivate') }}" class="space-y-4">
                @csrf
                <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-3 rounded-xl font-bold transition-colors">إعادة تفعيل الاشتراك</button>
            </form>
        </div>
    </div>
</div>
